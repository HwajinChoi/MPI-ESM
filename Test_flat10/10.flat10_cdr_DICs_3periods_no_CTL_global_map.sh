...
f=addfile(dir0+"test_flat10_cdr_001_cfcs_ideal_r192x96_dissic_dep${dep}_18500101_${C}1231.nc","r")
flt_tot=f->dissic(:,0,:,:)
flt_tot_an=month_to_annual(flt_tot,1)
g=addfile(dir0+"test_flat10_cdr_001_cfcs_ideal_r192x96_dissicnat_dep${dep}_18500101_${C}1231.nc","r")
flt_nat=g->dissicnat(:,0,:,:)
flt_nat_an=month_to_annual(flt_nat,1)
h=addfile(dir0+"test_flat10_cdr_001_cfcs_ideal_r192x96_dissicant_dep${dep}_18500101_${C}1231.nc","r")
flt_ant=h->dissic(:,0,:,:)
...
plot=new(9,graphic)
 wks=gsn_open_wks("png",dir1+"Flat10_cdr_DICs_no_CTL_ANO_global_map_3periods_dep_${dep}")
 res                        =   True
 res@gsnFrame               =       False
 res@gsnDraw                =       False
 res@cnFillOn               =       True
 ...
 plot(0)=gsn_csm_contour_map(wks,Middle_First(0,:,:),res)
 res@gsnCenterString=""
 plot(3)=gsn_csm_contour_map(wks,Middle_First(1,:,:),res)
 ;res@lbLabelBarOn           = True
 plot(6)=gsn_csm_contour_map(wks,Middle_First(2,:,:),res)
 res@gsnCenterString="(b) Last - First"
 res@lbLabelBarOn           =      False
 plot(1)=gsn_csm_contour_map(wks,Last_First(0,:,:),res)
 res@gsnCenterString=""
 plot(4)=gsn_csm_contour_map(wks,Last_First(1,:,:),res)
 ;res@lbLabelBarOn           = True
 plot(7)=gsn_csm_contour_map(wks,Last_First(2,:,:),res)
 ...
 plot(2)=gsn_csm_contour_map(wks,Last_Middle(0,:,:),res)
 res@gsnCenterString=""
 plot(5)=gsn_csm_contour_map(wks,Last_Middle(1,:,:),res)
 ;res@lbLabelBarOn           = True
 plot(8)=gsn_csm_contour_map(wks,Last_Middle(2,:,:),res)
 ...
 pres@gsnPanelMainString="Flat10-cdr anomaly (${A})"
