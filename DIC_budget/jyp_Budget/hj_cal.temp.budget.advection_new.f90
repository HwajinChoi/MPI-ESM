...
REAL, PARAMETER       :: pi=3.141592, re=6370000, undef=-1e20
...
OPEN (2, file='/data/hjchoi/CMI/OBA/data/gdata/new_u_199001-201712.gdat',form='unformatted',access='direct',recl=lx*ly*lz*4, status='old')
OPEN (3, file='/data/hjchoi/CMI/OBA/data/gdata/new_v_199001-201712.gdat',form='unformatted',access='direct',recl=lx*ly*lz*4, status='old')
...
open (21, file='/data/hjchoi/CMI/OBA/data/gdata/new.temp.zadv.199001-201712.gdat', form='unformatted', access='direct',recl=lx*ly*lz*4, status='unknown')
open (22, file='/data/hjchoi/CMI/OBA/data/gdata/new.temp.madv.199001-201712.gdat', form='unformatted', access='direct',recl=lx*ly*lz*4, status='unknown')
...
DO j=1, ly
DO i=1, lx
if(temp(i,j,k) .ne. undef .and. temp(i+1,j,k) .ne. undef .and. temp(i-1,j,k) .ne. undef) then
if(i==1) then
  zadv(i,j,k)=-u(i,j,k)*((temp(i+1,j,k)-temp(lx,j,k))/(2*cos(lat(j))*(pi*re/180)))
elseif (i==lx) then
  zadv(i,j,k)=-u(i,j,k)*((temp(1,j,k)-temp(i-1,j,k))/(2*cos(lat(j))*(pi*re/180)))
else
  zadv(i,j,k)=-u(i,j,k)*((temp(i+1,j,k)-temp(i-1,j,k))/(2*2*cos(lat(j))*(pi*re/180)))
endif
else
zadv(i,j,k)=undef
endif
...
DO j=1, ly
DO i=1, lx
if(temp(i,j,k) .ne. undef .and. temp(i,j+1,k) .ne. undef .and. temp(i,j-1,k) .ne. undef) then
if(j==1) then
  madv(i,j,k)=-v(i,j,k)*((temp(i,j+1,k)-temp(i,j,k))/(pi*re/180))
elseif (j==ly) then
  madv(i,j,k)=-v(i,j,k)*((temp(i,j,k)-temp(i,ly-1,k))/(pi*re/180))
else
  madv(i,j,k)=-v(i,j,k)*((temp(i,j-1,k)-temp(i,j+1,k))/(2*pi*re/180))
endif
else
madv(i,j,k)=undef
endif
...
!where(COUNT(temp(:,:,:,:) .ne. undef, DIM=4) .ne. 0)
if(k==1) then
!  vadv(:,:,k,:)=-wt2(:,:,k,:)*((temp(:,:,k+1,:)-temp(:,:,k,:))/(lev(k)-lev(k+1)))
  vadv(i,j,k)=-wt2(i,j,k)*((temp(i,j,k+1)-temp(i,j,k))/(lev(k)-lev(k+1)))
elseif(k==lz) then
!  vadv(:,:,k,:)=-wt2(:,:,k,:)*((temp(:,:,k,:)-temp(:,:,k-1,:))/(lev(k-1)-lev(k)))
  vadv(i,j,k)=-wt2(i,j,k)*((temp(i,j,k)-temp(i,j,k-1))/(lev(k-1)-lev(k)))
else
!  vadv(:,:,k,:)=-wt2(:,:,k,:)*((temp(:,:,k+1,:)-temp(:,:,k-1,:))/(lev(k-1)-lev(k+1)))
