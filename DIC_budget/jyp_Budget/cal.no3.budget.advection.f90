...
INTEGER, PARAMETER    :: lx=360, ly=180, lz=50, lt=552
...
if(i==1) then
  zadv(i,j,k,it)=-u(i,j,k,it)*((no3(i+1,j,k,it)-no3(lx,j,k,it))/(cos(lat(j))*(pi*re/180)))
elseif (i==lx) then
  zadv(i,j,k,it)=-u(i,j,k,it)*((no3(1,j,k,it)-no3(i-1,j,k,it))/(cos(lat(j))*(pi*re/180)))
else
...
open (11, file='zadv.core.gdat', form='unformatted', status='unknown')
!open (11, file='zadv.core.3Drelax.5dy.gdat', form='unformatted', status='unknown')
write(11) mzadv
close (11)
open (12, file='madv.core.gdat', form='unformatted', status='unknown')
!open (12, file='madv.core.3Drelax.5dy.gdat', form='unformatted', status='unknown')
write(12) mmadv
close (12)
open (13, file='vadv.core.gdat', form='unformatted', status='unknown')
!open (13, file='vadv.core.3Drelax.5dy.gdat', form='unformatted', status='unknown')
write(13) mvadv
close (13)
