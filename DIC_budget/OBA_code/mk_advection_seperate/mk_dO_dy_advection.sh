...
;-----------------------------------------------------------------------------------------
h=addfile(dir1+"v_${i}mAVG_ATMassim_10NS_100err_WOA_0.1_2nd.199001-201712.nc","r")
v=h->v
;-----------------------------------------------------------------------------------------
...
v_prime_O_bar=new((/dim(0),dim(1),dim(2)/),"float")
   ...
   delete([/i/])
    ...
    print(v_bar_O_prime(0,:,:))
    exit

system("rm -f "+dir2+"ECDA-COBALT_dO_dy_${i}_advection_199001-201712.nc")
out=addfile(    dir2+"ECDA-COBALT_dO_dy_${i}_advection_199001-201712.nc","c")
