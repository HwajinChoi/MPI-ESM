...
for v in "o2" "u" "v" "temp" "wt" ;do
 ...
 B=${dir1}/${w}
    for i in "1";do #Lead time
        for n in `seq 1992 ${i} 2017` ;do #output file
        cat <<EOF > ./mk_predict_vari.ncl
        begin
        m=${n}-1 ; input file
        I=${i}+1 ; Real lead time
        f=addfile("${dir0}/${v}.ensmean."+m+"01.nc","r")
        print("Input file : ${v}.ensmean."+m+"01.nc")
